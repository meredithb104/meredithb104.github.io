...
The site is the portfolio piece: the source is meant to be read. It is the sibling of [Commons UI](https://github.com/meredithb104/commons-ui), my React component library, and shares its token pipeline and testing philosophy. Where the site needs a pattern the library has, it uses the library's framework-free element rather than its own copy.

## What's in it

| Piece | Where | What it shows |
| --- | --- | --- |
| Design tokens with contrast enforcement | [`tokens/tokens.json`](tokens/tokens.json), [`scripts/build-tokens.ts`](scripts/build-tokens.ts) | Any color token can declare what it sits on and the ratio it needs. The build computes 86 pairs across three themes with the WCAG 2.x formula and exits non-zero if one fails. The `brand` stops (cake batter, white, monaco blue, yearbook blue, royal navy) declare no contrast target because they are the tile and the rules, except the tile's letters, which are a checked pair (`onBrand1`); the same colors used as text (`text`, `primary`, `accent`) are declared against every surface they sit on. Errors use a plain red outside the palette, and the token's `$note` says why. Runs directly under Node 24 with native type stripping; no build step for the build step. |
...
| Seven light-DOM custom elements, plus Commons UI's | [`src/components/`](src/components/) | `<theme-picker>`, `<font-picker>` (Public Sans by default, self-hosted and preloaded; Atkinson Hyperlegible Next fetched only when chosen; System as a no-web-font option), `<contrast-checker>`, `<work-filter>`, `<site-nav>` (current-section marking and the wiring for two menus: the phone-width Menu and the "More information" overflow, both [Commons UI](https://github.com/meredithb104/commons-ui)'s MenuButton as its framework-free `<cui-menu-button>`, installed from that repository; the site imports the element and its stylesheet, and the panels, closing on focus leaving, the layout-dependent `role="menu"`/`menuitem` on whichever list is the popup, and current-section marks are this element's), `<carousel-slider>`, `<page-tools>` (Previous section, Next section, Back to top: one small landmark after `main`, hidden until you scroll past the hero, never hidden from assistive technology because the Tab key is shared). Each enhances HTML that already works; none uses shadow DOM, so tokens and the focus ring apply everywhere. From Commons UI, as framework-free elements with their stylesheets layered into the site's cascade ([`src/styles/commons-ui.css`](src/styles/commons-ui.css)): `<cui-menu-button>` (both menus), `<cui-tabs>` (the Lab), `<cui-text-field>` (the contrast checker's hex fields), `<cui-live-region>` with `announce()`, and the `cui-button`, `cui-skip-link`, and `cui-visually-hidden` classes. |
| APG tabs and a tabbed carousel | [`index.html`](index.html), [`carousel-slider.ts`](src/components/carousel-slider.ts), [`roving.ts`](src/lib/roving.ts) | Tabs with automatic activation upgraded from a list of links, as Commons UI's `<cui-tabs>`. A carousel that never auto-rotates: focus stays on Previous/Next and a polite live region announces only "Slide 3 of 12: heading" (a live region around the whole track was far too chatty in JAWS), the slide picker is a roving tablist, hidden slides leave the tree and the tab order. The "How it's built" tab on the site explains the focus decision and where the pattern stops applying. |
| Live-region announcer | [`index.html`](index.html), Commons UI's `<cui-live-region>` | Polite and assertive regions in the page markup, empty from the start, cleared before each message so repeats are announced; `announce()` re-creates the element if it is ever missing. |
...
The whole site is one HTML document per page, one CSS file, and one JavaScript module (about 12 kB, 4 kB gzipped). One self-hosted, preloaded web font (Public Sans, 27 KB); no analytics, no third-party requests. The theme is applied by a two-line inline script before first paint, so there is no flash. Section highlighting uses `IntersectionObserver`, so nothing runs on scroll.
...
src/lib/                         contrast math, fragments, roving tabindex
src/components/                  custom elements (light DOM, progressive enhancement)
src/styles/                      base.css (layers, reset, focus), commons-ui.css (the library's styles, layered), site.css
