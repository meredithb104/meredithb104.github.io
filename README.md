...
| --- | --- | --- |
| Design tokens with contrast enforcement | [`tokens/tokens.json`](tokens/tokens.json), [`scripts/build-tokens.ts`](scripts/build-tokens.ts) | Any color token can declare what it sits on and the ratio it needs. The build computes 86 pairs across three themes with the WCAG 2.x formula and exits non-zero if one fails. The six `brand` stops (summer burgundy, cake batter, white, monaco blue, yearbook blue, royal navy) declare no contrast target because they are the tile and the rules; the same colors used as text (`text`, `primary`, `accent`) are declared against every surface they sit on, and where one has to move to pass (the burgundy lifted to a rose for dark-theme errors) the token's `$note` says so. Runs directly under Node 24 with native type stripping; no build step for the build step. |
