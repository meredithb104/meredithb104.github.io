...
The whole site is one HTML document per page, one CSS file, and one JavaScript module (about 12 kB, 4 kB gzipped). No web font by default (two optional ones are self-hosted and fetched only when chosen), no analytics, no third-party requests. The theme is applied by a two-line inline script before first paint, so there is no flash. Section highlighting uses `IntersectionObserver`, so nothing runs on scroll.
