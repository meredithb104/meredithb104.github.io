...
| --- | --- | --- |
| Design tokens with contrast enforcement | [`tokens/tokens.json`](tokens/tokens.json), [`scripts/build-tokens.ts`](scripts/build-tokens.ts) | Any color token can declare what it sits on and the ratio it needs. The build computes 86 pairs across three themes with the WCAG 2.x formula and exits non-zero if one fails. The six `brand` stops (tangerine, sage, plum, aqua, caramel, blush) declare no contrast target because as given they are surfaces and rules; where one becomes text (`primary` from the aqua, `accent` from the tangerine) it is deepened until the pair passes, and the token's `$note` says by how much. Runs directly under Node 24 with native type stripping; no build step for the build step. |
